...
While the `ghcr.io/retyc/retyc-k8s-csi` package is private, the nodes need a pull secret in the release namespace:
...
helm install ... --set imagePullSecrets[0].name=ghcr
```
...
| `imagePullSecrets` | `[]` | Pull secrets for the driver image, both components; needed while the ghcr.io package is private |
