...
1. **Provisioning.** A PVC with `storageClassName: retyc-rwx` (or `retyc-rwx-retain`) makes `csi-provisioner` call
   ...
   `<pv-name>` is reused.
2. **Staging.** When a pod using the claim lands on a node, kubelet calls `NodeStageVolume`. The plugin waits for its
   local WebDAV server (bounded, 30 s), then runs
   `mount -t davfs -o dir_mode=0777,file_mode=0666 http://127.0.0.1:8888/dataroom/<title> <globalmount>`.
...
`retyc webdav serve` exposes every dataroom the identity can see under `/dataroom/<title>`, decrypting on read and
encrypting on write with the account's AGE identity. It listens on `127.0.0.1:8888` inside the node plugin's network
namespace, without WebDAV authentication: the only client that can reach it is the `davfs2` mount started from the
same container. Nothing on the node or in other pods can connect to it.

The supervisor restarts the server five seconds after any exit and keeps its last output line, which is what `/readyz`
reports when the server is down (for instance `key passphrase check failed: wrong key passphrase`).
...
Steady state is about 40 MiB for the node plugin. Resolving a dataroom session in `retyc webdav serve` unlocks the
account's AGE key with scrypt (work factor 2^18, ~256 MiB of working memory), once per dataroom per process; the Go
runtime returns that memory within minutes. The manifests set a 512 MiB limit for that reason: a lower limit would
OOM-kill the plugin on the first mount, and with it every mount on the node.
...
| Wrong token or passphrase | node and controller pods go `1/2` NotReady with the reason on `/readyz`; a controller rollout keeps the previous pod | fix the Secret, restart the DaemonSet/Deployment |
